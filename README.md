...
## [gherkin-critic](./gherkin-critic)

Audits an existing Gherkin/Cucumber-js/Playwright suite against gherkin-genius's standards and writes the findings to a single markdown report — it never edits, fixes, or deletes a feature file, step definition, or support file.

It flags the same UI-layer violations gherkin-genius guards against while writing (steps that import application code, hit a database or internal API directly, assert on app/store state instead of the rendered page, or run on a non-Playwright driver) as blocking findings, alongside coverage gaps against a ticket or spec and structural/style issues in the feature files themselves. Findings are categorized by severity — blocking, major, minor — and written to a path the user specifies; actually fixing anything found is a separate follow-up with gherkin-genius.
Scenarios are written declaratively, describing behavior rather than UI mechanics, and every step is required to actually drive the browser through Playwright — no importing application code, no reaching into the database or an internal API to shortcut past the UI, no assertions against app/store state instead of what's rendered on the page. The skill also checks existing project conventions (tags, `World`/hook setup, existing steps) before adding anything new, so it extends a project's BDD suite instead of forking its style.
