...
- **No manipulating browser storage directly as a shortcut** — `page.evaluate(() => localStorage.setItem(...))`, `context.addCookies(...)`, writing to `sessionStorage`/IndexedDB, or `addInitScript` to plant a token/session before the page loads. This is still "using Playwright" in a literal sense, but it's the same violation as an internal API shortcut: it fabricates state instead of exercising the UI. If the app stores its auth token in `localStorage` after a real login, the test still has to log in through the actual form and let the app set that value itself — not set it directly and skip the form.
- **Assertions read from the UI**, not from application state, a store, a database row, or browser storage. `Then` steps check what's rendered on the page (text, visibility, an element's state) — not `expect(store.getState().foo).toBe(...)` or `expect(await page.evaluate(() => localStorage.getItem('x')))`.
...
- **Manipulating browser storage as a login/state shortcut** — `page.evaluate()` writing to `localStorage`/`sessionStorage`, `context.addCookies()`, or `addInitScript()` planting a session token instead of driving a real login/checkout/setup flow through the UI.
- **Asserting against application/store/database state instead of what's rendered** — a `Then` step should read the page, not the app's internals or its storage.
...
4. The exact command used to verify the scenario runs (and its result), not just an assertion that it "should work."        Given a cart totaling $99
        When the customer checks out
        Then shipping is charged at the standard rate
